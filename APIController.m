...
@interface APIController ()
{
    ...
    id<DataLoadedDelegate> dataLoadedDelegate;
}
...
+(APIController *) getInstance
{
    ...
    if (!instance) {
        instance = [[APIController alloc] init];
    }
    ...
    switch (self.state) {
        ...
        case courseRooms:
            if ([self.elementNameFromXMLParser isEqualToString:@"CourseType"]) {
                ...
                currentCourse.relativeURL = string;
            } else if ([self.elementNameFromXMLParser isEqualToString:@"ID"]) {
                stringToAppend = @"ID: ";
                currentCourse.identifier = string;
                ...
                currentCourse.status = [string intValue];
...
{
    if(dataLoadedDelegate)
    {
        [dataLoadedDelegate dataLoaded:self];
    }
}
